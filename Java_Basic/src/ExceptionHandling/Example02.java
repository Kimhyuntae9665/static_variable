...
//		문자열의 길이를 가져오는 String API method 
		int strLength = comment.trim().length();   //배열.length 는 배열의 길이 
...
//		첫번쨰로 검색되는 인덱스 위치를 반환 
		int stringIndex = comment.indexOf("dolor");
		System.out.println(stringIndex);
		
//		해당하는 문자열과 지정 문자열을 비교해서 결과를 출력하는 String API method
		boolean flag = comment.equals(comment);
		System.out.println(flag);
		
//		문자열의 앞뒤 공백을 제거하는 Strning API method 
